 ...
 * 
 * Inspired by @earendil-works/pi-ai design:
 * - Single interface across Anthropic, OpenAI, Google, etc.
 * - Streaming support
 * - Model alias resolution ("sonnet" → "anthropic/claude-sonnet-4-20250514")
 */
...
export type StreamEvent =
  ...
  | { type: "finish"; finishReason: string; usage?: ChatResult["usage"] }
...
export function resolveModel(modelOrAlias: string, overrides?: Partial<ModelConfig>): ModelConfig {
  ...
  if (slashIdx > 0) {
    return {
      provider: modelOrAlias.slice(0, slashIdx),
      model: modelOrAlias.slice(slashIdx + 1),
      ...overrides,
    }
  }

  // Fallback: treat as model name with default deepseek provider
  return {
    provider: "deepseek",
    model: modelOrAlias,
    ...overrides,
...
export async function chat(options: ChatOptions): Promise<ChatResult> {
  ...
  if (!provider) {
    throw new Error(
      `Unknown provider "${options.model.provider}". Available: ${[...providers.keys()].join(", ")}`
    )
  }
