...
export default function Home() {
  ...
  const selectSession = useCallback(async (id: string) => {
    setActiveId(id)
    // Load from Messages API — has full artifacts/files/toolCalls metadata.
    // Query API lacks artifacts/files and collapses multi-turn structure.
    try {
        ...
        for (var i = 0; i < raw.length; i++) {
          ...
          if (m.role === "assistant") {
            ...
            if (artifacts && artifacts.length > 0) {
              for (var ai2 = 0; ai2 < artifacts.length; ai2++) {
                sitems.push({ id: artifacts[ai2].id, type: "artifact", content: artifacts[ai2].content, artifactTitle: artifacts[ai2].title, artifactType: artifacts[ai2].type, artifactFileUrl: artifacts[ai2].fileUrl, artifactStreaming: false })
              }
            }
      ...
      {sidebarOpen ? (
        <aside className="w-60 bg-bg-secondary border-r border-border flex flex-col shrink-0">
          ...
          <div className="border-t border-border p-2 space-y-0.5">
            <a href="/dashboard" target="_blank" className="block px-3 py-1.5 rounded-lg text-xs text-text-muted hover:text-text-secondary hover:bg-bg-hover transition-colors">📊 Dashboard</a>
            <a href="/settings" target="_blank" className="block px-3 py-1.5 rounded-lg text-xs text-text-muted hover:text-text-secondary hover:bg-bg-hover transition-colors">⚙️ Settings</a>
          </div>
